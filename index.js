...
console.log(color(figlet.textSync("GacorBot", {
font: 'DOS Rebel',
...
✉️  Script Base - Version 1.0
✉️  Author : YangJunMing
✉️  Gmail : shinesebastian255@gmail.com
✉️  Instagram : https://www.instagram.com/shinesannn2008

${chalk.green.bold("Script Dibuat Oleh ShinySebastian :D")}\n`));

async function startBotz() {
const { state, saveCreds } = await useMultiFileAuthState("GacorSession")
...
require("./gabutcmd")(Rafael, m, chatUpdate, store)
} catch (err) {
console.log(err)
}
})
...
Rafael.sendVideoAsSticker = async (jid, path, quoted, options = {}) => {
let buff = Buffer.isBuffer(path) ? path : /^data:.*?\/.*?;base64,/i.test(path) ? Buffer.from(path.split`,`[1], 'base64') : /^https?:\/\//.test(path) ? await (await getBuffer(path)) : fs.existsSync(path) ? fs.readFileSync(path) : Buffer.alloc(0)
let buffer
if (options && (options.packname || options.author)) {
...
fs.watchFile(file, () => {
fs.unwatchFile(file)
console.log(`File telah diupdate: ${__filename}`)
